...
class _GithubLoginState extends State<GithubLoginWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('OAuth2 Github Login'),
      ),
      body: Center(
  ...
  Future<oauth2.Client> _getOAuth2Client(Uri redirectUrl) async {
    if (widget.githubClientId.isEmpty || widget.githubClientSecret.isEmpty) {
      throw GithubLoginException(
          'Please set githubClientId and githubClientSecret in lib/github_oauth_credentials.dart');
    }
...
class GithubLoginException implements Exception {
  GithubLoginException(this.message);
